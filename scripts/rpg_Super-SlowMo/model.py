...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.padding is None:
            dim = int(np.ceil(x.shape[2] / 32) * 32), int(np.ceil(x.shape[3] / 32) * 32)
            pad_h = -(x.shape[2] - dim[0])
            pad_w = -(x.shape[3] - dim[1])
            self.padding = torch.nn.ReplicationPad2d([pad_w//2, pad_w-pad_w//2, pad_h//2, pad_h-pad_h//2])
            self.cropping = torch.nn.ReplicationPad2d([-pad_w//2, -(pad_w-pad_w//2), -pad_h//2, -(pad_h-pad_h//2)])
