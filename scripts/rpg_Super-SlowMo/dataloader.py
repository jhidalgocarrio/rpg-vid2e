...
def _make_video_dataset(dir, img_range, keyframe):
    """
    ...
    if keyframe is not None:
        keyframe_idx = frames.index(keyframe)
        min_idx = max([keyframe_idx-img_range, 0])
        max_idx = min([keyframe_idx+img_range+1,len(frames)])
    else:
        min_idx = 0
        max_idx = len(frames)
    ...
    for idx in range(min_idx, max_idx):
        # Add path to list.
        framesPath.append(os.path.join(dir, frames[idx]))
    return framesPath

def _pil_loader(path, cropArea=None, resizeDim=None, frameFlip=0, resizeFactor=1):
    """
    ...
    with open(path, 'rb') as f:
        ...
        W, H = img.size
        if resizeDim == None:
            resizeDim = (int(W*resizeFactor), int(H*resizeFactor))
...
class Video(data.Dataset):
    ...
    def __init__(self, root=None, pandas_dataframe_file=None, transform=None, img_range=None, keyframe=None, fps=30, downsample_factor=1):
        """
        Parameters
        ----------
            root : string
        ...
        if root is not None:
            framesPath = _make_video_dataset(root, img_range, keyframe)
            self.framesPath = framesPath
            ...
            frame        = _pil_loader(framesPath[0], resizeFactor=1.0/downsample_factor)

            self.origDim = frame.size

        else:
            if pandas_dataframe_file is not None:
                self.df = pd.read_pickle(pandas_dataframe_file)
                frame = self.df["image"].iloc[0]
            else:
                raise ValueError("Either pandas_dataframe_file should be known or root.")

            self.origDim = frame.shape

        self.dim     = int(np.ceil(self.origDim[0] / 32)) * 32, int(np.ceil(self.origDim[1] / 32)) * 32
        self.root           = root
        self.transform      = transform
    ...
    def __getitem__(self, index):
        ...
        if len(self.framesPath) > 0:
            for framePath in [self.framesPath[index], self.framesPath[index + 1]]:
                # Open image using pil.
                image = _pil_loader(framePath, resizeDim=self.origDim)
                # Apply transformation if specified.
                if self.transform is not None:
                    image = self.transform(image)

                # add padding
                px = (self.dim[0]-self.origDim[0])//2
                py = (self.dim[1]-self.origDim[1])//2
                image = torch.from_numpy(np.pad(image.numpy(), ((0,0), (py,py), (px,px)), mode="reflect"))
                sample.append(image)
            times = [index * 1.0/self.fps, (index+1) * 1.0/self.fps]
        else:
            for i in [index, index+1]:
                image = self.df["image"].iloc[i]

                if self.transform is not None:
                    image = self.transform(image)

                sample.append(image)
                times.append(self.df["time"].iloc[i])
